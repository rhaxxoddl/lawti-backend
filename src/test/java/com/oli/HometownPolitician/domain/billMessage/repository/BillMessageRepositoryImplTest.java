...
@ActiveProfiles("test")
@Transactional
class BillMessageRepositoryImplTest {
    ...
    @Test
    @DisplayName("메세지 방의 메세지들을 불러오는지 확인")
    void qFindByBillId_well_test() {
    ...
    private BillMessageListInput getBillMessageListInput() {
        ...
        BillMessageListInput billMessageListInput = BillMessageListInput.builder()
                .billId(bills.get(0).getId())
                .pagination(TargetSlicePaginationInput.from(null, 5, true))
                .build();
        return billMessageListInput;
    }
    ...
    private List<Bill> insertBillData(List<Committee> committeeList) {
        ...
        bills.add(Bill.builder()
                ...
                .billExternalId("testExternalBillId3")
                .number(1234568L)
                .proposeDate(LocalDate.now())
